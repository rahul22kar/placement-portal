...
class Projects extends Component {
    ...
    render() {
        const { onEditProject, handleRemoveProject, loading, data} = this.props;
        const projectData = [];
        data.map(item =>
            projectData.push({
                project_id: item._id,
        ...
        const Projects = () => projectData.map(project =>
            (<Card
                ...
                extra={
                    <div>
                        <Icon type='edit' style={{fontSize: 15, marginRight: 15}} onClick={() => onEditProject(project)}/>
                        <Icon type='minus-circle' style={{fontSize: 15}} onClick={() => handleRemoveProject(project.project_id)}/>
                    </div>
                }
                style={{
                    margin: 5
